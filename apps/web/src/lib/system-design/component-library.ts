/**
 * Shared vocabulary of system-design building blocks. This file is safe to import from both
 * server code (API routes) and client components: it only describes generic component
 * *metadata* (label, category, icon, blurb) - never a problem's canonical tree or hints, which
 * live in `catalog.server.ts` and never reach the client bundle.
 */
